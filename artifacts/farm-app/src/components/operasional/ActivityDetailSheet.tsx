import { 
  CalendarDays, ChevronDown, Bug, Activity, 
  TrendingUp, LeafyGreen, ShieldAlert 
} from "lucide-react";
...
// Perluasan tipe untuk mengakomodasi data dinamis dari backend
type RichAgronomyItem = AgronomyItem & {
  metaEkstra?: Record<string, any>;
};

interface ActivityDetailSheetProps {
  item: RichAgronomyItem | null;
  onClose: () => void;
  onStatusChange?: (id: string, status: string) => void;
}

export function ActivityDetailSheet({ item, onClose, onStatusChange }: ActivityDetailSheetProps) {
  if (!item) return null;
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <>
                      <div className="rounded-2xl border border-border/60 bg-card p-3 shadow-sm">
                        <div className="flex items-center gap-2 text-muted-foreground mb-1">
                          ...
                          <span className="text-xs font-bold uppercase">Umur Tanaman</span>
                        </div>
                        <p className="text-lg font-black">{item.metaEkstra.hst || 0} <span className="text-sm font-medium text-muted-foreground">HST</span></p>
                      </div>

                      <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-3 shadow-sm">
                         <div className="flex items-center gap-2 text-red-600 mb-2">
                          <Bug className="h-4 w-4" />
                          <span className="text-xs font-bold uppercase">Hama / Penyakit</span>
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {item.metaEkstra.hama?.map((h: string) => <Badge key={h} variant="destructive" className="text-[10px] rounded-sm py-0 bg-red-500/80">{h}</Badge>)}
                          {item.metaEkstra.penyakit?.map((p: string) => <Badge key={p} variant="destructive" className="text-[10px] rounded-sm py-0 bg-orange-500/80">{p}</Badge>)}
                          {(!item.metaEkstra.hama?.length && !item.metaEkstra.penyakit?.length) && <span className="text-xs font-medium text-emerald-600">Aman Terkendali</span>}
                        </div>
                      </div>
                    </>
                  )}
                  ...
                  {item.module === "finance" && item.metaEkstra.nominal !== undefined && (
                    <div className="col-span-2 rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-4 shadow-sm flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs font-bold uppercase text-emerald-700/70">Estimasi Nilai Transaksi</p>
                          <p className="text-xl font-black text-emerald-700">Rp {item.metaEkstra.nominal.toLocaleString("id-ID")}</p>
                        </div>
                      </div>
                    </div>
                  )}
                  ...
                  {item.module === "perawatan" && item.metaEkstra.tags && (
                    <div className="col-span-2 rounded-2xl border border-border/60 bg-card p-3 shadow-sm flex items-center gap-3">
                       ...
                       <span className="text-sm font-medium">Kategori Treatment: <strong className="font-black">{item.metaEkstra.tags}</strong></span>
                    </div>
                  )}
            ...
            <section className="mt-6 space-y-3 pb-8">
              ...
              <div className="flex flex-wrap gap-2">
                {item.workers.map((worker) => (
                  <Badge key={worker} variant="outline" className="rounded-full px-3 py-1 shadow-sm">{worker}</Badge>
                ))}
              </div>
            </section>
