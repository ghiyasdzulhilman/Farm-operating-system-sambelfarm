...
import { 
  Leaf, Plus, FileText, Loader2, TrendingUp, MoreHorizontal, 
  Sprout, Wrench, ChevronRight, CloudUpload, CheckCircle2, Layers 
} from "lucide-react";
...
export function AgronomyHubPage() {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status }: { id: string; status: string }) => {
      // Endpoint ini nantinya akan kita buat di backend untuk update properti status di Notion
      // Untuk sementara, kita asumsikan sukses agar UI Optimistic berjalan
      // return fetch(`/api/operasional/${id}/status`, { method: "PATCH", body: JSON.stringify({ status }) });
      console.log(`Mengubah status ${id} menjadi ${status}`);
    ...
    onError: (err, variables, context) => {
      ...
      toast({ variant: "destructive", title: "Gagal mengubah status", description: "Cek koneksi internet Anda." });
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: ["operasional-feed"] });
      toast({ title: "Status Diperbarui", description: "Perubahan telah disinkronkan ke Notion." });
    }
  });
        ...
        <div className="flex items-center justify-between rounded-2xl border border-amber-500/30 bg-amber-500/10 px-5 py-4 text-amber-700 shadow-sm dark:text-amber-400">
          <div className="flex items-center gap-4">
            ...
            <div className="space-y-0.5">
              <p className="text-sm font-bold">Ada {meta.stagingCount} Aktivitas di Antrean Offline</p>
              <p className="text-xs opacity-80">Data aman di lokal. Tekan sync saat sinyal kebun stabil.</p>
            </div>
          </div>
          <Button 
            size="sm" 
